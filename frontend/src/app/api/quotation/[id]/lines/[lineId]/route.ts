// ===========================================
// DealFlow360 - Quotation Line Detail API
// ===========================================

import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/lib/db';
import { getSession } from '@/lib/auth';
import { ActorType } from '@/lib/types';
import { updateQuotationLineSchema } from '@/lib/validators';
import { auditLogger, evaluateQuotation } from '@/lib/services';

interface RouteParams {
  params: Promise<{ id: string; lineId: string }>;
}

// PUT /api/quotation/[id]/lines/[lineId] - Update line item
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    const session = await getSession();
    if (!session?.user || session.user.actorType !== ActorType.INTERNAL) {
      return NextResponse.json(
        { success: false, error: { code: 'UNAUTHORIZED', message: 'Authentication required' } },
        { status: 401 }
      );
    }

    const { id: quotationId, lineId } = await params;
    const body = await request.json();
    const parsed = updateQuotationLineSchema.safeParse(body);

    if (!parsed.success) {
      return NextResponse.json(
        { success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid input', details: parsed.error.flatten() } },
        { status: 400 }
      );
    }

    // Verify quotation and line exist
    const quotation = await prisma.quotation.findUnique({ where: { id: quotationId } });
    if (!quotation) {
      return NextResponse.json(
        { success: false, error: { code: 'NOT_FOUND', message: 'Quotation not found' } },
        { status: 404 }
      );
    }

    if (quotation.status !== 'DRAFT') {
      return NextResponse.json(
        { success: false, error: { code: 'INVALID_STATE', message: 'Lines can only be updated on draft quotations' } },
        { status: 400 }
      );
    }

    const existingLine = await prisma.quotationLine.findUnique({
      where: { id: lineId },
      include: { product: true },
    });

    if (!existingLine || existingLine.quotationId !== quotationId) {
      return NextResponse.json(
        { success: false, error: { code: 'NOT_FOUND', message: 'Line not found' } },
        { status: 404 }
      );
    }

    const { quantity, unitPrice, discountPct, lineType, billingFrequency } = parsed.data;

    // Calculate new values
    const newQuantity = quantity ?? existingLine.quantity;
    const newUnitPrice = unitPrice ?? existingLine.unitPrice.toNumber();
    const newDiscountPct = discountPct ?? existingLine.discountPct.toNumber();
    const discountedPrice = newUnitPrice * (1 - newDiscountPct / 100);
    const lineTotal = discountedPrice * newQuantity;
    const costTotal = existingLine.product.costPrice.toNumber() * newQuantity;
    const marginAmount = lineTotal - costTotal;
    const marginPct = lineTotal > 0 ? (marginAmount / lineTotal) * 100 : 0;

    const line = await prisma.quotationLine.update({
      where: { id: lineId },
      data: {
        quantity: newQuantity,
        unitPrice: newUnitPrice,
        discountPct: newDiscountPct,
        lineTotal,
        lineType: lineType ?? existingLine.lineType,
        billingFrequency: billingFrequency ?? existingLine.billingFrequency,
        marginAmount,
        marginPct,
      },
      include: {
        product: { select: { name: true, category: true } },
      },
    });

    // Update quotation totals
    await updateQuotationTotals(quotationId);

    await auditLogger.logUpdate(
      session.user.id,
      ActorType.INTERNAL,
      'QUOTATION_LINE',
      lineId,
      { quantity: existingLine.quantity, unitPrice: existingLine.unitPrice, discountPct: existingLine.discountPct },
      { quantity: line.quantity, unitPrice: line.unitPrice, discountPct: line.discountPct }
    );

    return NextResponse.json({
      success: true,
      data: {
        id: line.id,
        productId: line.productId,
        productName: line.product.name,
        productCategory: line.product.category,
        quantity: line.quantity,
        unitPrice: line.unitPrice.toNumber(),
        discountPct: line.discountPct.toNumber(),
        lineTotal: line.lineTotal.toNumber(),
        lineType: line.lineType,
        billingFrequency: line.billingFrequency,
        marginAmount: line.marginAmount.toNumber(),
        marginPct: line.marginPct.toNumber(),
      },
      message: 'Line updated successfully',
    });
  } catch (error) {
    console.error('[Quotation/Lines/Update] Error:', error);
    return NextResponse.json(
      { success: false, error: { code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' } },
      { status: 500 }
    );
  }
}

// DELETE /api/quotation/[id]/lines/[lineId] - Remove line item
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    const session = await getSession();
    if (!session?.user || session.user.actorType !== ActorType.INTERNAL) {
      return NextResponse.json(
        { success: false, error: { code: 'UNAUTHORIZED', message: 'Authentication required' } },
        { status: 401 }
      );
    }

    const { id: quotationId, lineId } = await params;

    const quotation = await prisma.quotation.findUnique({ where: { id: quotationId } });
    if (!quotation) {
      return NextResponse.json(
        { success: false, error: { code: 'NOT_FOUND', message: 'Quotation not found' } },
        { status: 404 }
      );
    }

    if (quotation.status !== 'DRAFT') {
      return NextResponse.json(
        { success: false, error: { code: 'INVALID_STATE', message: 'Lines can only be removed from draft quotations' } },
        { status: 400 }
      );
    }

    const existingLine = await prisma.quotationLine.findUnique({ where: { id: lineId } });
    if (!existingLine || existingLine.quotationId !== quotationId) {
      return NextResponse.json(
        { success: false, error: { code: 'NOT_FOUND', message: 'Line not found' } },
        { status: 404 }
      );
    }

    await prisma.quotationLine.delete({ where: { id: lineId } });

    // Update quotation totals
    await updateQuotationTotals(quotationId);

    await auditLogger.logDelete(session.user.id, ActorType.INTERNAL, 'QUOTATION_LINE', lineId, {
      quotationId,
      productId: existingLine.productId,
      quantity: existingLine.quantity,
    });

    return NextResponse.json({
      success: true,
      message: 'Line removed successfully',
    });
  } catch (error) {
    console.error('[Quotation/Lines/Delete] Error:', error);
    return NextResponse.json(
      { success: false, error: { code: 'INTERNAL_ERROR', message: 'An unexpected error occurred' } },
      { status: 500 }
    );
  }
}

// Helper function to update quotation totals and recalculate risk score
async function updateQuotationTotals(quotationId: string) {
  const lines = await prisma.quotationLine.findMany({
    where: { quotationId },
  });

  const totalAmount = lines.reduce((sum, line) => sum + line.lineTotal.toNumber(), 0);
  const totalMargin = lines.reduce((sum, line) => sum + line.marginAmount.toNumber(), 0);
  const totalMarginPct = totalAmount > 0 ? (totalMargin / totalAmount) * 100 : 0;

  // Recalculate risk score whenever lines change
  let blendedRiskScore = 0;
  if (lines.length > 0) {
    try {
      const riskResult = await evaluateQuotation(quotationId);
      blendedRiskScore = riskResult.blendedScore;
    } catch (error) {
      console.error('[Quotation/Lines] Risk score calculation failed:', error);
    }
  }

  await prisma.quotation.update({
    where: { id: quotationId },
    data: {
      totalAmount,
      totalMargin,
      totalMarginPct,
      blendedRiskScore,
      lastActivityAt: new Date(),
    },
  });
}
